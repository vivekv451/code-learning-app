...
export default function LearnProApp() {
  ...
  const lookupSchool = async (code: string) => {
    ...
    try {
      const data = await apiCall(`/schools/lookup?code=${code}`);
      setRegSchoolData(data);
      showToast(`School found: ${data.name}`);
    } catch (error) {
      setRegSchoolData(null);
    }
  };
      ...
      {activeScreen === 'screen-chapter' && selectedChapter && (
        <div className="bg-gray-50 pb-20">
          ...
          <div className="p-4">
            ...
            <div className="bg-white rounded-2xl border-2 border-gray-100 overflow-hidden shadow-md mb-4">
              <div className="px-4 py-3 bg-navy flex items-center gap-3">
                <span className="text-2xl">✏️</span>
                <div><div className="text-xs font-bold text-white/50 uppercase">Practical Task</div><div className="font-bold text-white text-sm">Code Challenge</div></div>
              </div>
              <div className="px-4 py-3 bg-blue-50 border-b border-gray-100 text-sm text-blue-900 font-semibold">{selectedChapter.practical_tasks}</div>
              <div className="relative bg-navy p-4">
                <textarea className="w-full min-h-32 bg-navy text-blue-300 font-mono text-sm outline-none resize-none" placeholder="Write your code here..."></textarea>
              </div>
              <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-100">
                <span className="text-xs text-gray-400">Ready? Test your code</span>
                <button className="bg-green-600 text-white font-bold text-xs px-4 py-2 rounded-lg hover:bg-green-700">Run ▶</button>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => goScreen('screen-course')} className="flex-1 py-3 border-2 border-gray-200 bg-white rounded-xl text-sm font-bold hover:border-blue-400 transition">← Chapters</button>
